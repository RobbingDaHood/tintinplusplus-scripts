...
container_name="ProceduralRealsNeo4j"
docker start $container_name || docker run \
--publish=7475:7474 --publish=7688:7687 \
--volume=$HOME/neo4j/procedural_realms/data:/data \
--env=NEO4J_AUTH=none \
--name=$container_name \
...
output="$(curl -X POST -H Accept:application/json -H Content-Type:application/json -u neo4j:testneo4j http://localhost:7475/db/neo4j/tx/commit -d "$body" 2> /dev/null)"
